...
def zenGarden(bg):
    ...
    while out!="quit":
        ...
        for i in range (len(plants)):
            r=plantRects[i]
            if r.collidepoint(lcx,lcy) and tool==plants[i].state==0:
                tool="none"
                plants[i].state=5
                plants[i].water+=1
                plants[i].passed=clock()
                allcoins.append(Coin(0,r[1]-10,\
                                  randint(r[0]-10,r[0]+r[2]+10),\
                                  r[1]+r[-2]+10,0,clock()))
            elif r.collidepoint(lcx,lcy) and tool==plants[i].state==1:
                tool="none"
                plants[i].state=5
                plants[i].age+=1
                plants[i].passed=clock()
                for c in range (plants[i].age):
                    allcoins.append(Coin(1,r[1]-10,\
                                      randint(r[0]-10,r[0]+r[2]+10),\
                                      r[1]+r[-2]+10,0,clock()))
                    
            elif r.collidepoint(lcx,lcy) and tool==plants[i].state:
                plants[i].state=4
                tool="none"
                allcoins.append(Coin(1,r[1]-10,\
                                  randint(r[0]-10,r[0]+r[2]+10),\
            ...
            if clock()-plants[i].passed>24 and plants[i].state==4:
            ...
            if plants[i].state==5:
                if clock()-plants[i].passed>5:
                    if plants[i].water<3:
                        plants[i].state=0
                    elif plants[i].age<3:
                        plants[i].state=1
                    elif plants[i].age>=3:
                        plants[i].state=randint(2,3)
